...
;(function(root, factory) {
    ...
    jeDt.initDate = function(opts) {
        ...
        var createDiv = function(disCell, self) {
            ...
            // 个人修改
            var outDiv = doc.createElement('div');
            outDiv.className = "out-jedate";
            //个人修改
            ...
            //个人修改
            disCell.appendChild(outDiv);
            outDiv.appendChild(dateDiv);
            // 个人修改
    ...
    jeDt.setHtml = function() {
        ...
        var bothmsStr = !isYYMM ? '<div class="botflex jedatehmsshde"><ul class="jedatehms"><li><input type="text" /></li><i>:</i><li><input type="text" /></li><i>:</i><li><input type="text" /></li></ul></div>' + '<div class="botflex jedatebtn"><span class="jedateok">确认</span><span class="jedatetodaymonth">今天</span><span class="jedateclear">取消</span></div>' :(dateFormat == "YYYY" ? '<div class="botflex jedatebtn"><span class="jedateok" style="width:47.8%">确认</span><span class="jedateclear" style="width:47.8%">取消</span></div>' : '<div class="botflex jedatebtn"><span class="jedateok">确认</span><span class="jedatetodaymonth">本月</span><span class="jedateclear">清空</span></div>');
...
// console.log(year, month,jeDt.attr(jeDt.find(".jedateyear")[0], "year"))
    ...
    jeDt.chooseDays = function() {
        jeDt.bind(jeDt.find(".jedaul li"), "click", function(ev) {
            ...
            // console.log(that, liTms, valcell)
            ...
            // 个人修改代码处
            $(that).addClass('action').siblings('li').removeClass('action');
            // 个人修改代码处
        });
    };
    //生成定位时分秒
    ...
    jeDt.events = function(tmsArr) {
        ...
        if (!isYYMM) {
            ...
            !ishhmmss && jeDt.each([ mPre, mNext ], function(i, cls) {
                jeDt.bind(cls, "click", function(ev) {
                    ...
                    var year = parseInt(jeDt.attr(jeDt.find(".jedateyear")[0], "year"));
                    var month = parseInt(jeDt.attr(jeDt.find(".jedatemonth")[0], "month")-1);
                    // console.log(year,month)
                    cls == mPre ?jeDt.getDateStr(year, --month) : jeDt.getDateStr(year, ++month);
                });
            });
            //时分秒事件绑定
            ...
            jeDt.bind(jeDt.find(".jedatebot .jedatetodaymonth"), "click", function() {
                ...
                var gettoDate = jeDt.parse([ toTime[0], toTime[1], toTime[2] ], [ toTime[3], toTime[4], toTime[5] ], jeDt.format);
                var valcell = jeDt.elemCell;
                jeDt.getDateStr(toTime[0], toTime[1], toTime[2]);
                // console.log(toTime, gettoDate,end_time)
        ...
        jeDt.bind(jeDt.find(".jedatebot .jedateok"), "click", function(ev) {
            ...
            // 个人修改代码处
            var li = $('.jedaul li.action'),
                year = li.attr('year'),
                month = li.attr('month'),
                day = li.attr('day'),
                okVal = year+'-'+ month + '-' + day;
            ...
            // 个人修改代码处
            if (jeDt.isType(jeDt.opts.choosefun, "function") || jeDt.opts.choosefun != null) jeDt.opts.choosefun&&jeDt.opts.choosefun(jeDt.elemCell,okVal);
            // 个人修改代码处
        });
        //点击空白处隐藏
