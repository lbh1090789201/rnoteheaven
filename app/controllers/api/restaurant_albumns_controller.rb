include BdLbsHelper
module Api
  class RestaurantAlbumnsController < ApiController
    #before_filter :authenticate_user_from_token!, except: [:forgot_password]
    # before_filter :authenticate_user!

    #上传餐厅信息
    def create
      restaurantALbumn = RestaurantAlbumn.new(
                                  :image =>restaurantAlbumn_params[:image],
                                  :restaurant_id =>restaurantAlbumn_params[:restaurant_id])

      unless restaurantALbumn.save
        puts '餐厅信息上传失败'
        render json: {
                   success: false,
                   errors: ['餐厅信息上传失败'],
      ...
      fcf = {
          id: restaurantALbumn.id,
          image: restaurantALbumn.image,
      }
      render json: {
                 success: true,
                 info: '餐厅信息上传成功',
